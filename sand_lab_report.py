...
import pandas as pd
import qrcode
from fpdf import FPDF  # With fpdf2 installed, this will import the new library
...
def build_main_view():
    ...
    with st.expander("📋 Step-by-Step Protocol", expanded=False):
        ...
        5. **Methylene Blue**: Titrate with MB solution. The MBV is calculated and used to estimate clay content.
...
def process_new_sample(inputs: SampleData):
    ...
    try:
        ...
        st.error(f"💥 Failed to generate PDF report. Error: {e}")
        return
    finally:
